...
# I first used a recursive function to run the simulation, but this final version relays on a While loop.
...
def print_field(matrix):  # Prints a 0/1 matrix as a field (" "/"X" and borders)
    print("+"+"-"*len(matrix[0])+"+")
    for line in matrix:
        print("|"+"".join(str(x) for x in line).replace("0", " ").replace("1", "X")+"|")  # Uses replace method
    print("+"+"-"*len(matrix[0])+"+")
...
def iterate(matrix):  # Takes a matrix and returns the matrix of the next generation
...
while True:  # Runs the code below continuously until user stops it
    ...
    while answer != "n" and answer != "y":
        ...
        answer = input()
    if answer == "y":
        field = iterate(field)  # Compute next state of the field and update the working one
    else:
